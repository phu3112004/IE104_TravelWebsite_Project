...
import { faStar,faAngleRight,faAngleDown,faAngleUp } from "@fortawesome/free-solid-svg-icons";
                ...
                <div key={index} className={cx("des-content-item")}>
                  {item.topthingtodo.map((todo, todoIndex) => (
                    <div key={todoIndex}>
                      ...
                      >
                        <h3>{todo.title}</h3>
                        <FontAwesomeIcon icon={ details[todoIndex] ? faAngleUp : faAngleDown  } />
                      </button>
                      {/* Chỉ hiển thị nội dung nếu trạng thái là true */}
                      {details[todoIndex] && (
